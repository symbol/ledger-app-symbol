...
void display_tx(uint8_t *raw_tx, uint16_t dataLength,
    ...
    uint32_t networkGenerationHash0 = getUint32(&raw_tx[disIndex]);
    uint32_t networkGenerationHash1 = getUint32(&raw_tx[disIndex+4]);
    uint32_t networkGenerationHash2 = getUint32(&raw_tx[disIndex+8]);
    uint32_t networkGenerationHash3 = getUint32(&raw_tx[disIndex+12]);
    uint32_t networkGenerationHash4 = getUint32(&raw_tx[disIndex+16]);
    uint32_t networkGenerationHash5 = getUint32(&raw_tx[disIndex+20]);
    uint32_t networkGenerationHash6 = getUint32(&raw_tx[disIndex+24]);
    uint32_t networkGenerationHash7 = getUint32(&raw_tx[disIndex+28]);

    txContent.txType = getUint16(reverseBytes(&raw_tx[disIndex + networkGenerationHashLength + 2], 2));
    // Load dataLength of tx
    if ((txContent.txType == 0x4141) || (txContent.txType == 0x4241)){
        if ((networkGenerationHash0 == GENERATION_HASH0) && (networkGenerationHash1 == GENERATION_HASH1) &&
            (networkGenerationHash2 == GENERATION_HASH2) && (networkGenerationHash3 == GENERATION_HASH3) &&
            (networkGenerationHash4 == GENERATION_HASH4) && (networkGenerationHash5 == GENERATION_HASH5) &&
            (networkGenerationHash6 == GENERATION_HASH6) && (networkGenerationHash7 == GENERATION_HASH7)){
            tmpCtx.transactionContext.rawTxLength = 84;
        } else {
            tmpCtx.transactionContext.rawTxLength = 32;
        }
    } else {
